...
/** 树节点类型。 */
type TreeNode =
  | { kind: 'folder'; name: string; handle: FileSystemDirectoryHandle; children: TreeNode[]; loaded: boolean }
...
function DeleteConfirmDialog({
  ...
  return (
    ...
    >
      <div
        ref={dialogRef}
        className="w-80 rounded-xl border border-[#30363d] bg-[#161b22] p-5 shadow-2xl"
      >
        <h3 className="text-sm font-medium text-[#e6edf3]">确认删除</h3>
        <p className="mt-3 text-xs text-[#8b949e]">
          确定要删除 <span className="font-medium text-[#e6edf3]">{fileName}</span> 吗？
        </p>
        <p className="mt-2 rounded bg-amber-500/10 px-2 py-1.5 text-xs text-amber-400">
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button
            ...
            className="rounded px-3 py-1.5 text-xs text-[#8b949e] transition-colors hover:bg-[#30363d] hover:text-[#e6edf3]"
          >
            取消
          </button>
          <button
...
function RenameDialog({
  ...
  return (
    ...
    >
      <div className="w-80 rounded-xl border border-[#30363d] bg-[#161b22] p-5 shadow-2xl">
        <h3 className="text-sm font-medium text-[#e6edf3]">重命名</h3>
        <input
          ref={inputRef}
          type="text"
          value={name}
          ...
          className="mt-3 w-full rounded border border-[#30363d] bg-[#0d1117] px-2 py-1.5 text-xs text-[#e6edf3] outline-none focus:border-[#165DFF]"
        />
        <div className="mt-4 flex justify-end gap-2">
          <button
            type="button"
          ...
          <button
            ...
            className="rounded bg-[#165DFF] px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-[#3c7dff] disabled:opacity-50"
          >
            确定
          </button>
        </div>
    ...
    <div>
      <div
        ...
        className={`group flex cursor-pointer items-center gap-1 rounded px-2 py-1 text-xs transition-colors ${
          isDragOver
            ? 'bg-[#165DFF]/20 ring-1 ring-[#165DFF]'
            : isHighlighted
              ? 'bg-[#165DFF]/20 text-[#58a6ff]'
        ...
        {hovered && (
          <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
            <button
              ...
              className="rounded p-0.5 text-[#8b949e] hover:bg-[#30363d] hover:text-[#e6edf3]"
              title="重命名"
            ...
            <button
              ...
              className="rounded p-0.5 text-[#8b949e] hover:bg-[#30363d] hover:text-red-400"
              title="删除"
      ...
      {isFolder && isExpanded && (
        <div role="group">
          {node.children.length === 0 ? (
            <div
              className="py-1 text-xs text-[#484f58]"
...
export function FileTree({ onOpenFile, activeTabName }: FileTreeProps): JSX.Element {
  ...
  // ── 文件夹懒展开 ──
  const handleToggleFolder = useCallback(async (handle: FileSystemDirectoryHandle, path: string) => {
  ...
  // ── 任务 23：新建文件/文件夹 ──
  const handleNewItem = useCallback(async (name: string): Promise<OpResult> => {
  ...
  const handleRename = useCallback(async (newName: string): Promise<OpResult> => {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent, targetNode: TreeNode) => {
    ...
    try {
      ...
      if (sourceLookup.node.kind === 'file') {
        ...
        // 复制文件夹到目标
        const newDirHandle = await targetNode.handle.getDirectoryHandle(sourceName, { create: true });
  ...
  if (!fsaReady) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center" data-testid="filetree-unavailable">
        <p className="text-xs leading-relaxed text-[#8b949e]">当前浏览器不支持文件夹访问</p>
        <p className="mt-1 text-xs text-[#484f58]">请使用 Chromium 内核浏览器</p>
      </div>
    );
  }
  ...
  if (!rootHandle) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center" data-testid="filetree-empty">
        <p className="text-xs leading-relaxed text-[#8b949e]">授权文件夹后可浏览文件</p>
        <p className="mt-1 text-xs text-[#484f58]">支持桌面/文档等位置，可随时撤销</p>
        <button
          type="button"
          onClick={() => void handleGrant()}
          data-testid="filetree-grant-btn"
          className="mt-3 rounded-lg bg-[#165DFF] px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-[#3c7dff]"
        >
          授权文件夹
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col" data-testid="filetree">
      ...
      <div className="mb-2 flex items-center gap-1">
        <span className="truncate text-xs font-medium text-[#e6edf3]">{rootHandle.name}</span>
        <button
          type="button"
          onClick={() => setNewItemMode('file')}
          disabled={loading}
          data-testid="tree-new-file-btn"
          className="ml-auto rounded p-1 text-[#8b949e] transition-colors hover:bg-[#30363d] hover:text-[#e6edf3] disabled:opacity-50"
          title="新建文件"
        >
          +
        </button>
      ...
      {/* 错误提示 */}
      {error && (
        <div className="mb-2 rounded bg-red-500/10 px-2 py-1 text-xs text-red-400" role="alert" data-testid="filetree-error">
          {error}
        </div>
      )}
      ...
      {/* 加载中 */}
      {loading ? (
        <div className="py-4 text-center text-xs text-[#484f58]" data-testid="filetree-loading">
          加载中…
        </div>
      ) : tree.length === 0 ? (
        <div className="py-4 text-center text-xs text-[#484f58]">文件夹为空</div>
      ) : (
