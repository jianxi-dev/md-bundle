/* Command palette search engine — pure, DOM-free matching + recency ordering.
 *
 * Split out of command-palette.ts so the palette module stays under the
 * 250-pure-LOC ceiling. NOT part of the package public API: only
 * command-palette.ts imports this file.
 */
...
/** Record a command id as most-recently-used. */
...
/** Canonical palette group order. `其他` catches commands with no group. */
const GROUP_ORDER: readonly string[] = ['格式', '块', '插入', '视图', '体检', '其他'];

/**
 * Stable-partition entries into canonical group order. Within a group the
 * incoming order (recent-first, then score) is preserved. Groups outside the
 * canonical set render after it in first-seen order. Returned in grouped
 * order so the palette's selection index maps to its rendered row order.
 */
function orderByGroup(entries: MatchEntry[]): MatchEntry[] {
  const buckets = new Map<string, MatchEntry[]>();
