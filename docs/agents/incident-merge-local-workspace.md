...
上游 mdpkg 修复 docx 导出 8 项缺陷后，将新构建的 web/mdpkg-web.js 同步至本仓库 vendor（PR #96）。
合并成功后收尾执行了：
...
**后果**：本地工作区文件（apps/web/vendor/mdpkg-web.js）随 checkout 还原为旧版本，
dev server 直接从磁盘加载该文件 → 本地验证「修复未生效」→ 误判为部署/修复失败，排查成本翻倍。
...
- Vendored engine 同步：apps/web/vendor/mdpkg-web.js（上游 jianxi-dev/mdpkg 构建产物）
